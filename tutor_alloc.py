...
m.setObjective(
    1 / np.sum(N_w) * quicksum(P_iw[i][w] * X_iw[i, w] for i in Tutors for w in Workshops if (i, w) in X_iw)
    + weight / 2 * ((1 / np.sum(N_w == 2)) * quicksum(Y_ijw[i, j, w] * Div_ij[i, j] for (i, j, w) in Y_ijw)
                    + (1 / 3 / np.sum(N_w == 3)) * quicksum(Y_ijw[i, j, w] * Div_ij[i, j] for (i, j, w) in Y_ijw))
    , GRB.MAXIMIZE)
